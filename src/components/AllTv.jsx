...
export default function AllTVShows({ lang }) {
  ...
  useEffect(() => {
    const fetchAllTVShows = async () => {
      setLoading(true);
      try {
        const tvShowsData = await fetchData(`/discover/tv`, lang, filters);
        const formattedTVShows = tvShowsData.results.map((show) => ({
          id: show.id,
          title: show.name,
          imageUrl: `https://image.tmdb.org/t/p/w500${show.poster_path}`,
          genre: show.adult ? 'Adult' : 'Kids',
          additionalInfo: show.first_air_date.split('-')[0],
        }));
      ...
      <div className="flex justify-center mb-6 md:mb-0 md:max-w-xs w-full md:mr-4">
        <div className="dark:bg-gray-800 bg-gray-200 text-white p-6 rounded-lg shadow-lg w-full">
          ...
          <div className="flex flex-col">
            ...
            {/* First Air Date Year */}
            <label className="block text-sm font-medium mb-1 text-black dark:text-white">
              {lang === 'ar' ? 'سنة العرض الأول' : 'First Air Date Year'}
            </label>
            <select
              className="w-full p-2 border rounded mb-4 dark:bg-gray-700 bg-gray-100 dark:text-white text-black"
